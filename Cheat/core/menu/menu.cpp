...
void c_menu::run()
{
	...
	static bool trigger_enabled = checkModuleActive(globals::user_modules, MODULE_TRIGGER_BOT);
	static bool bhop_enabled = checkModuleActive(globals::user_modules, MODULE_BUNNY_HOP);
	static bool wallhack_enabled = checkModuleActive(globals::user_modules, MODULE_WALLHACK);
	static bool changer_enabled = checkModuleActive(globals::user_modules, MODULE_SKIN_CHANGER);

	if (opened)
	{
		ImGui::SetNextWindowSize(ImVec2(568, 698), ImGuiSetCond_Always);
		ImGui::PushFont(font_menu);
		ImGui::Begin(XorStr("ALPHA CHEAT [beta] SP1"), &opened, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
		{
			if (ImGui::BeginTabBar(XorStr("##main_tabs"), ImGuiTabBarFlags_None))
			{
				if (aim_enabled || trigger_enabled)
				{
					if (ImGui::BeginTabItem(XorStr("AIM BOT")))
					{
						...
						{
							if (ImGui::BeginTabBar(XorStr("##aim_bot_tabs")))
							{
								if (aim_enabled)
								{
							...
							if (ImGui::BeginTabBar(XorStr("##misc_tabs"), ImGuiTabBarFlags_None))
							{
								...
								if (wallhack_enabled) 
								{
									if (ImGui::BeginTabItem(XorStr("OTHER MISC")))
									{
										...
										ImGui::Checkbox(XorStr("rank reveal"), &config_system.get_config().misc.rank_reveal);
				...
				if (changer_enabled)
				{
					if (ImGui::BeginTabItem(XorStr("CHANGER")))
					{
						ImGui::BeginChild(XorStr("##changer_window"));
						{
							...
							{
								if (ImGui::BeginTabItem(XorStr("WEAPONS & KINFE")))
								{
									ImGui::PushFont(font_main);
									ImGui::Checkbox("skinchanger enable", &config_system.get_config().skin_changer.enable_skinchanger);
									ImGui::Spacing();
									ImGui::Spacing();
									if (config_system.get_config().skin_changer.enable_skinchanger)
									{
										ImGui::Columns(2);
										ImGui::BeginChild(XorStr("##knife_window"));
										{
											ImGui::Text(XorStr("KNIFE SKINS"));
											ImGui::Spacing();
											...
											ImGui::Spacing();
											if (ImGui::Button(XorStr("apply to knife"))) {
										...
										ImGui::NextColumn();
										ImGui::BeginChild(XorStr("##weapons_window"));
										{
											ImGui::Text(XorStr("WEAPON SKINS"));
											ImGui::Spacing();
											...
											if (ImGui::Button(XorStr("apply to weapon")))
											{
								...
								if (ImGui::BeginTabItem(XorStr("GLOVES ")))
								{
									...
									if (config_system.get_config().skin_changer.enable_glovechanger)
									{
										...
										ImGui::Spacing();
										if (ImGui::Button(XorStr("apply gloves"))) {
											utilities::force_update();
										}
									}
									ImGui::PopFont();
							...
							if (ImGui::BeginTabItem(XorStr("CONFIG SYSTEM")))
							{
									...
									)
								{
									...
									if (ImGui::Button((XorStr("create"))))
									{
										...
										config_system.save(config_system.get_configs().size() - 1);
										save_config = true;
									}
