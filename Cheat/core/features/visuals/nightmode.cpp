...
void c_nightmode::run() noexcept {
	...
	static auto r_drawspecificstaticprop = interfaces::console->get_convar(XorStr("r_DrawSpecificStaticProp"));
	r_drawspecificstaticprop->set_value(config_system.item.nightmode ? 0 : 1);

	static auto r_3dsky = interfaces::console->get_convar(XorStr("r_3dsky"));
...
void c_nightmode::apply() noexcept {
	...
	old_sky_name = interfaces::console->get_convar(XorStr("sv_skyname"));
