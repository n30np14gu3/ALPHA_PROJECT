...
void c_trigger::trigger(c_usercmd* user_cmd) {
	...
	if(config_system.current_weapon->trigger_settings.on_key && !GetAsyncKeyState(config_system.current_weapon->trigger_settings.key_id))
		return;
	...
	if (config_system.current_weapon->trigger_settings.rcs)
	...
	if (config_system.current_weapon->trigger_settings.hitbox_head)
	{
		if (hitgroup == HITGROUP_HEAD) didhit = true;
	}
	if (config_system.current_weapon->trigger_settings.hitbox_body)
	{
		if (hitgroup == HITGROUP_CHEST || hitgroup == HITGROUP_STOMACH) didhit = true;
	}
	if (config_system.current_weapon->trigger_settings.hitbox_arms)
	{
		if (hitgroup == HITGROUP_LEFTARM || hitgroup == HITGROUP_RIGHTARM) didhit = true;
	}
	if (config_system.current_weapon->trigger_settings.hitbox_legs)
	{
		if (hitgroup == HITGROUP_LEFTLEG || hitgroup == HITGROUP_RIGHTLEG) didhit = true;
	}

	if (trigger_delay >= 2 + config_system.current_weapon->trigger_settings.delay && didhit)
	{
		trigger_delay = 0;
